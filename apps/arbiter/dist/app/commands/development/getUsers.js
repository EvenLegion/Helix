...
//#region src/app/commands/development/getUsers.ts
const command = {
	name: "get-users",
...
async function chatInput({ interaction }) {
	...
	const filteredMembers = guild.members.cache.filter((member) => member.roles.cache.has(ROLE_ID)).map((member) => ({
		...
		roles: member.roles.cache.filter((role) => selectedRole.includes(role.id)).map((role) => role.name).join("-"),
		id: member.user.id
	}));
	const csvHeader = "Username,Nickname,Roles,ID";
	const csvRows = filteredMembers.map((m) => `"${m.username}","${m.nickname}","${m.roles}","${m.id}"`);
	...
	setTimeout(() => fs.unlinkSync(filePath), 1e4);
}
