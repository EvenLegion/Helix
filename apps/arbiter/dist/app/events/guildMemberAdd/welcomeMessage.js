...
//#region src/app/events/guildMemberAdd/welcomeMessage.ts
async function welcomeMessage_default(member) {
	try {
		const welcomeChannel = member.guild.channels.cache.find((channel) => channel.id === "1198735581186895892");
		if (!welcomeChannel || !welcomeChannel.isTextBased()) return;
		const embed = new EmbedBuilder().setColor("#FF3131").setTitle("Welcome to the Even Legion!").setDescription(`Hello <@${member.user.id}>, welcome to ${member.guild.name}! \n \n`).setFields([
			{
				name: "Server-Rules",
...
//#endregion
export { welcomeMessage_default as default };
